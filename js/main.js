...
const date = new Date("06/30/2023");
...
function mapSetup() {
  ...
  map.attributionControl.addAttribution(
    'Map Data &copy; <a href="https://github.com/ginseng666/GeoJSON-TopoJSON-Austria/">Flooh Perlot</a>' +
      ' ,  Infection Data &copy; <a href="https://orf.at/corona/daten/bezirke">ORF.at</a> & <a href="https://www.data.gv.at/covid-19">data.gv.at</a>' +
...
function findFileOpenIfFound(toCheckDate, map, info) {
  const fileName =
    ...
    toCheckDate.getDate() +
    "." +
    (toCheckDate.getMonth() + 1) +
    "." +
    toCheckDate.getFullYear() +
    ".csv";
  $.ajax({
    url: fileName,
    type: "HEAD",
  ...
  Papa.parse(file, {
    ...
    complete: function (results) {
      ...
      if (results.data[0]["GKZ"] !== undefined) {
        ...
        finalData = combiner(
          sevenDayChange(results),
          0.5,
          oneDayChange(results),
          0.5
        );
      ...
      for (let i = 0; i < finalData.length; i++) {
        if (
          bezirke_999_geo["features"][i]["properties"]["name"] ===
          results.data[i]["Bezirk"]
        ) {
      ...
      console.log(
        "Worst municipality on the " +
          loadedDay.getDate() +
          "." +
          (loadedDay.getMonth() + 1) +
          "." +
          loadedDay.getFullYear()
      );
...
function combiner(data1, weight1, data2, weight2) {
  ...
  for (let i = 0; i < data1.length; i++) {
    resultArray[i] =
      Math.round(
        ((data1[i] * weight1 + data2[i] * weight2) * (5 / 70) +
          Number.EPSILON) *
          100
      ) / 100;
  }
...
function sevenDayChangeAbsoluteNumbers(argument) {
  ...
  for (let i = 0; i < argument.data.length; i++) {
    cache =
      (100000 / argument.data[i]["AnzEinwohner"]) *
      argument.data[i]["AnzahlFaelle7Tage"];

    resultArray[i] =
      (typeof cache || false) === "string"
        ? parseFloat(cache.replace(",", ".")) * 2
        : cache * 2;
  }
  return resultArray;
}
